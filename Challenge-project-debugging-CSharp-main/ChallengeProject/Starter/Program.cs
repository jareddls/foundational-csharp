...
static void MakeChange(int cost, int[] cashTill, int twenties, int tens = 0, int fives = 0, int ones = 0)
{
    ...
    //our money in the register, we add what they gave us
    cashTill[3] += twenties;
    cashTill[2] += tens;
    cashTill[1] += fives;
    cashTill[0] += ones;

    //the amount the cusotomer paid
    ...
    if (changeNeeded < 0){
        //we return back to the till the money if they don't have enough to cover the money needed
        cashTill[3] -= twenties;
        cashTill[2] -= tens;
        cashTill[1] -= fives;
        cashTill[0] -= ones;
    ...
    //5$ purchase
    //gave us 21$ (1 twenty 1 one)
//1140
//1124
    if (changeNeeded > 0) {
        //if we can't complete this, we return the change we tried to make back in our till
        cashTill[3] += twentyCounter;
        cashTill[2] += tenCounter;
        cashTill[1] += fiveCounter;
        cashTill[0] += oneCounter;

        //then we take out the money they gave
        cashTill[3] -= twenties;
        cashTill[2] -= tens;
        cashTill[1] -= fives;
        cashTill[0] -= ones;
